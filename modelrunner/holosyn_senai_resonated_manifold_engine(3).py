    ...
    def load_weights(self, model_key: str = "fused_all") -> Optional[Any]:
        ...
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                try:
                    weights = torch.load(target_path, map_location=self.device, weights_only=True)
                except Exception:
                    try:
                        weights = torch.jit.load(target_path, map_location=self.device)
                    except Exception:
            ...
            try:
                with warnings.catch_warnings():
                    ...
                    try:
                        ...
                        try:
                            weights = torch.jit.load(target_path, map_location="cpu")
                        except Exception:
