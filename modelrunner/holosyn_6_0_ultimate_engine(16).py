...
class LocalSubconsciousSwarm:
    ...
    def switch_model(self, model_key_or_name: str) -> str:
        ...
        # Resolve preset key with automatic correction for common digit/letter typos (e.g. qweb 2.0b, qwen2v1 -> qwen2vl)
    ...
    def preload_models(self, keys: Optional[List[str]] = None) -> List[str]:
        """
        Pre-downloads and caches HuggingFace weights for local presets safely on Dell Latitude.
        Purges RAM after each model download to prevent OOM/crash before batch execution.
        """
        target_keys = keys or ["smollm", "smollm360m", "qwen0.5b", "qweb2.0b", "deepseek1.5b", "tinyllama", "opt125m", "minimax"]
        results = []
        print("\n 📥 INITIATING SAFE MODEL PRE-DOWNLOAD & CACHING SEQUENCE...")
        for k in target_keys:
            ...
            if target_name == "OrganicQwenMinimaxFastDecoder":
                ...
                continue
            if not TRANSFORMERS_AVAILABLE or not TORCH_AVAILABLE:
                results.append(f"   ⚠️ [{k}]: Skipping download (Transformers/PyTorch unavailable)")
                continue
            try:
                print(f"   📥 Downloading & Verifying [{k}] -> {target_name}...")
                if "qwen2-vl" in target_name.lower():
                    if QWEN2VL_TRANSFORMERS_AVAILABLE:
                        _ = AutoProcessor.from_pretrained(target_name)
                        _ = Qwen2VLForConditionalGeneration.from_pretrained(target_name, torch_dtype=torch.float32, low_cpu_mem_usage=True)
                    else:
                        results.append(f"   ⚠️ [{k}]: Qwen2VL not supported in current transformers build")
                        continue
                else:
                    _ = AutoTokenizer.from_pretrained(target_name)
                    _ = AutoModelForCausalLM.from_pretrained(target_name, torch_dtype=torch.float32, low_cpu_mem_usage=True)
                self.purge_ram()
                results.append(f"   ✅ [{k}]: Cached successfully -> {target_name}")
            except Exception as e:
                self.purge_ram()
                results.append(f"   ⚠️ [{k}]: Download/Cache failed ({e}) -> High-Fidelity Fallback active")
        return results

    def ensure_swarm_active(self) -> str:
        """Verifies subconscious swarm health and ensures an active model is loaded."""
        if self.model is None or self.tokenizer is None:
            msg = self.switch_model("smollm")
            return f"   🐝 SWARM VERIFIED & ACTIVATED: {msg}"
