#!/usr/bin/env python3
"""
HOLOSYN V67: MASTER LINGUISTIC & CONTEXT-FREE OBSERVER (ENV-MAPPED)
===================================================================================
...
Patch: Updated JIT model ingestion paths to target /Intellibloomenv/lang/
"""
...
class TorchScriptLinguisticMicroSwarm:
    ...
    def _boot_distilled_heads(self):
        ...
        # 🛠️ PATCH: Targeted specific environment directory
        target_dir = "/home/devcbloom/Documents/Intellibloomenv/lang"
        
        for file in candidate_files:
            # Check relative local path and the absolute /lang path
            ...
            for p in paths:
                if os.path.exists(p):
                    try:
                        ...
                        print(f"   ⚠️ [JIT MICRO-MODEL] Failed to bind {file}: {e}")
    ...
    def extract_latent_resonance(self, text, fallback_snn):
        ...
        if not self.models:
            return float(np.mean(fallback_snn)) if fallback_snn and len(fallback_snn) > 0 else 0.5
...
class ZipfianMorphologyObserver(BaseObserver):
    ...
    def evaluate(self, s, sy, p, snn, text="", haptic_level=0.0, **kwargs):
        ...
        return float((morphological_density + zipfian_coherence) / 2.0)
...
class ChomskyanSyntacticObserver(BaseObserver):
    ...
    def evaluate(self, s, sy, p, snn, text="", haptic_level=0.0, **kwargs):
        safe_text = self.array_safe_text(text).lower()
        ...
        return float(parsing_efficiency)
...
class UnifiedLinguisticNexus(BaseObserver):
    ...
    def evaluate(self, s, sy, p, snn, text="", haptic_level=0.0, **kwargs):
        ...
        try:
            ...
            linguistic_coherence = float((lexical_coherence + parsing_efficiency + jit_latent_resonance) / 3.0)
