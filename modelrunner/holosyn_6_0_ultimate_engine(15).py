...
class LocalSubconsciousSwarm:
    ...
    def switch_model(self, model_key_or_name: str) -> str:
        ...
        key_lower = raw_input.lower()
        
        # Resolve preset key with automatic correction for common digit/letter typos (e.g. qwen2v1 -> qwen2vl)
        target_name = LOCAL_MODEL_PRESETS.get(key_lower)
        if not target_name:
            typo_fixed = key_lower.replace("qwen2v1", "qwen2vl").replace("v1", "vl")
    ...
    def preload_models(self, keys: Optional[List[str]] = None) -> List[str]:
        ...
        """
        target_keys = keys or ["smollm", "smollm360m", "qwen0.5b", "tinyllama", "opt125m", "minimax"]
        results = []
        print("\n 📥 INITIATING SAFE MODEL PRE-DOWNLOAD & CACHING SEQUENCE...")
        for k in target_keys:
            target_name = LOCAL_MODEL_PRESETS.get(k.lower(), k)
