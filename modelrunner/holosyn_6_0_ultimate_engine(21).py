...
# Suppress noisy OpenCV / V4L2 video device warnings
...
LOCAL_MODEL_PRESETS = {
    ...
    "qwen2v1": "Qwen/Qwen2-VL-2B-Instruct",                # Typo Alias (1 -> l)
    "qweb2.0b": "Qwen/Qwen2-VL-2B-Instruct",               # QWeb 2.0B Alias
    "qweb2b": "Qwen/Qwen2-VL-2B-Instruct",                 # QWeb 2B Short Alias
    "qwen2.0b": "Qwen/Qwen2-VL-2B-Instruct",               # Qwen 2.0B Alias
    "qwen2b": "Qwen/Qwen2-VL-2B-Instruct",                 # Qwen 2B Alias
    "qwen3b": "Qwen/Qwen2.5-3B-Instruct",                  # ~3B params   - High Capacity Logic
    "deepseek1.5b": "deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B", # ~1.5B params - Distilled Reasoning
    "gemma2b": "google/gemma-2-2b-it",                     # ~2.6B params - Fast Instruction
...
class LocalSubconsciousSwarm:
    ...
    Enforces aggressive RAM purging, strict Memory Lock guards, and zero-leak disk caching.
    """
    def __init__(self):
        ...
        self.auto_subconscious_active = False
    ...
    def check_memory_status(self) -> str:
        ...
        return f"💻 [DELL LATITUDE 5420 PROFILE]: CPU Threads={threads} | Memory Lock=ENGAGED | Max RAM Ceiling={self.max_ram_gb:.1f}GB | Active Model={self.current_model_name}"
    ...
    def apply_memory_lock(self):
        """Enforces a hard Memory Lock by purging untracked references, running GC, and trimming heap memory."""
        gc.collect()
        if TORCH_AVAILABLE and hasattr(torch, "cuda") and torch.cuda.is_available():
            torch.cuda.empty_cache()
        gc.collect()
    ...
    def switch_model(self, model_key_or_name: str) -> str:
        """Dynamically loads local model on CPU with strict Memory Lock and low memory overhead."""
        self.apply_memory_lock()
        raw_input = model_key_or_name.strip()
        key_lower = raw_input.lower().replace(" ", "").replace("_", "")
        
        # Resolve preset key with automatic correction for common digit/letter typos
        target_name = LOCAL_MODEL_PRESETS.get(key_lower)
        if not target_name:
            typo_fixed = (
                key_lower.replace("qweb2.0b", "qweb2b")
                .replace("qweb", "qwen")
                .replace("qwen2v1", "qwen2vl")
                .replace("v1", "vl")
            )
            target_name = LOCAL_MODEL_PRESETS.get(typo_fixed, raw_input)
        
        # Direct check for local simulated presets like MiniMax
        if target_name == "OrganicQwenMinimaxFastDecoder" or key_lower in ["minimax", "organicqwenminimaxfastdecoder"]:
            self.purge_ram()
            self.current_model_name = "OrganicQwenMinimaxFastDecoder"
            return "   ✅ DELL LATITUDE SIMULATED MINIMAX DECODER ACTIVE: [OrganicQwenMinimaxFastDecoder]"
    ...
    def preload_models(self, keys: Optional[List[str]] = None) -> List[str]:
        """
        Pre-downloads HuggingFace weights directly to disk cache without PyTorch tensor instantiation in RAM.
        Uses snapshot_download to prevent RAM accumulation crashes during bulk downloads.
        """
        target_keys = keys or ["smollm", "smollm360m", "qwen0.5b", "qweb2.0b", "deepseek1.5b", "tinyllama", "opt125m", "minimax"]
        results = []
        print("\n 🔒 INITIATING MEMORY-LOCKED ZERO-RAM DISK STREAM DOWNLOAD SEQUENCE...")
        
        for k in target_keys:
            clean_k = k.lower().replace(" ", "").replace("_", "")
            target_name = LOCAL_MODEL_PRESETS.get(clean_k, k)
            
            if target_name == "OrganicQwenMinimaxFastDecoder":
                results.append("   ✅ [MINIMAX SIMULATOR]: Ready (Built-in Simulator)")
                continue
                
            try:
                self.apply_memory_lock()
                print(f"   🔒 [MEMORY LOCK & BUS STREAM] Downloading [{k}] -> {target_name} to disk cache...")
                
                if HF_HUB_AVAILABLE:
                    # Stream files directly to HuggingFace disk cache without loading tensors into Python RAM
                    snapshot_download(repo_id=target_name, resume_download=True)
                else:
                    # Fallback tokenizer/processor fetch
                    if "qwen2-vl" in target_name.lower():
                        if QWEN2VL_TRANSFORMERS_AVAILABLE:
                            AutoProcessor.from_pretrained(target_name)
                    else:
                        AutoTokenizer.from_pretrained(target_name)
                        
                self.apply_memory_lock()
                results.append(f"   ✅ [{k}]: Downloaded & Cached to Disk -> {target_name}")
            except Exception as e:
                self.apply_memory_lock()
                results.append(f"   ⚠️ [{k}]: Download failed ({e}) -> High-Fidelity Fallback active")
                
        self.apply_memory_lock()
        return results

    def ensure_swarm_active(self) -> str:
        """Verifies subconscious swarm health and ensures an active model is loaded under Memory Lock."""
        if self.model is None or self.tokenizer is None:
            msg = self.switch_model("smollm")
            return f"   🐝 SWARM VERIFIED & MEMORY-LOCKED: {msg}"
        return f"   🐝 SWARM ACTIVE: Active Model = [{self.current_model_name}]"
    ...
    def generate_thought_pulse(self, governor_lock: str = "OMN", context_hint: str = "") -> str:
        ...
        pulses = [
            ...
            f"[SUBCONSCIOUS {self.current_model_name}]: Autonomous swarm consensus locked on {governor_lock} node."
        ]
...
class HolosynDynamic:
    ...
    def __init__(self, vault_path: Optional[str] = None, auto_harvest: bool = False):
        ...
        self.auto_subconscious_loop: bool = False
        self.auto_interval: float = 2.0
    ...
    def rebuild_manifold(self, path: str) -> str:
        """
        Harvests directory or file path for weight shards (.pt, .pth, .bin) and observer plugins (.py).
        Rebuilds core weights and dynamically registers all BaseObserver classes found in the path.
        """
        if not path:
            return " ❌ No path specified for vault rebuild."

        clean_path = path.strip(" []'\"")
        if not os.path.exists(clean_path):
            return f" ❌ Vault path '{clean_path}' does not exist."

        print(f" 📂 HARVESTING VAULT: {clean_path}")
        loaded_count = 0
        ...
        if os.path.isdir(clean_path):
            for root, _, files in os.walk(clean_path):
                for f in files:
                    if f.endswith((".pt", ".pth", ".bin", ".torchscript", ".py")):
                        target_files.append(os.path.join(root, f))
        else:
            target_files = [clean_path]

        py_plugins = [f for f in target_files if f.endswith(".py")]
        weight_files = [f for f in target_files if f.endswith((".pt", ".pth", ".bin", ".torchscript"))]

        if py_plugins:
            res_p = self.load_plugin(",".join(py_plugins))
            print(f"   {res_p}")
            loaded_count += len(py_plugins)

        for w_path in weight_files:
            try:
                if TORCH_AVAILABLE:
                    w_obj = torch.load(w_path, map_location="cpu", weights_only=False)
                    if "FOUNDATION" in self.cores:
                        self.cores["FOUNDATION"].assimilate_weights(w_obj)
                        loaded_count += 1
            except Exception:
                pass

        return f"   ✅ VAULT HARVEST COMPLETE: Processed {loaded_count} items from {clean_path}."

    def load_plugin(self, path_or_paths: str) -> str:
        """
        Manually loads one or multiple comma-separated observer .py plugin files or weight files.
        Sanitizes input paths and dynamically registers valid BaseObserver classes.
        """
        if not path_or_paths:
            return " ❌ No plugin paths specified."

        cleaned_input = (
            path_or_paths.replace("[", "")
            .replace("]", "")
            .replace("dated observers", "datedobservers")
            .strip()
        )
        ...
        for path in paths:
            ...
            if clean_path.endswith((".pt", ".pth", ".bin", ".torchscript")):
                try:
                    w_obj = torch.load(clean_path, map_location="cpu", weights_only=False) if TORCH_AVAILABLE else None
                    if w_obj and "FOUNDATION" in self.cores:
                        self.cores["FOUNDATION"].assimilate_weights(w_obj)
                        loaded_count += 1
                except Exception:
                    pass
            ...
            try:
                ...
                if spec and spec.loader:
                    ...
                    for attr in dir(plugin_module):
                        ...
                        if (
                            isinstance(obj, type)
                            and issubclass(obj, BaseObserver)
                            and obj is not BaseObserver
        ...
        return f"   ✅ PLUGIN BATCH LOAD COMPLETE: Successfully assimilated {loaded_count} plugin pathways."

    def load_batch(self, batch_key: str = "1") -> str:
        """
        Loads pre-configured batches of observer plugins by portion (1 to 5, or 'all').
        Automatically sanitizes folder paths and validates observer classes.
        """
        batch_map = {
            "1": [
        ...
        if not target_list:
            return f" ❌ Batch key '{key_str}' not recognized. Valid options: 1, 2, 3, 4, 5, or 'all'."
    ...
    def force_governor(self, key: Optional[str]) -> str:
        ...
        if not key or key.lower() in ["auto", "none", "clear"]:
            ...
            self.auto_gov_swarm_enabled = True
            return "   🔓 GOVERNOR LOCK: Autonomous Swarm Consensus Mode Engaged."
        
        key = key.upper()
        if key in self.observers:
            self.forced_governor = key
            self.auto_gov_swarm_enabled = False
            return f"   🔒 GOVERNOR LOCK: Forced lock engaged on [{key}]."
        return f"   ❌ Observer [{key}] does not exist."

    def execute_autonomous_subconscious_cycle(self, hint: str = "") -> Tuple[str, str, float, float]:
        """
        Executes a full autonomous subconscious swarm tick:
        1. Generates local subconscious thought pulse.
        2. Evaluates thought through active cores & hive observers.
        3. Dynamic Swarm Auto-Governance consensus resolution.
        """
        thought, truth_score, resonance = self.ai_interface.generate_subconscious_signal(
            governor_lock=self.forced_governor or "SWARM_AUTO",
            context_memory=hint or "Autonomous swarm pulse running."
        )
        voltages, uni, gov, scores, haptic, p = self.process(thought)
        
        # Auto-Governance Swarm Consensus Modulation
        if self.auto_gov_swarm_enabled and not self.forced_governor:
            # Shift weight towards highest resonating observer head
            best_head = max(scores.keys(), key=lambda k: scores[k]) if scores else "OMN"
            for h in self.observer_weights:
                self.observer_weights[h] *= 0.98
            self.observer_weights[best_head] = min(2.0, self.observer_weights.get(best_head, 1.0) + 0.05)
            
        summary = (
            f" 🧠 [AUTONOMOUS SUBCONSCIOUS SWARM TICK #{self.cycle}]\n"
            f" 🤖 Model       : {self.ai_interface.local_subconscious.current_model_name}\n"
            f" 💭 Thought     : '{thought}'\n"
            f" 🌀 Phase Sync  : {uni:+.5f} rad | 💓 Pulse: {p:+.4f} | Haptic: {haptic:.4f}\n"
            f" 👑 Swarm Gov   : {gov} (Auto-Swarm={'ON' if self.auto_gov_swarm_enabled else 'OFF'})"
        )
        return summary, thought, uni, haptic
    ...
    def get_status_report(self) -> str:
        ...
        lines = [
            ...
            f"  • Governor Lock   : {self.forced_governor or ('AUTONOMOUS SWARM CONSENSUS' if self.auto_gov_swarm_enabled else 'MANUAL')}",
            f"  • Auto Subconscious: {'ENABLED' if self.auto_subconscious_loop else 'DISABLED'} (Interval: {self.auto_interval:.1f}s)",
...
def start_cli():
    ...
    print(" AUTONOMOUS SUBCONSCIOUS & GOVERNANCE:")
    print("    /auto [on|off|interval]  : Toggle continuous autonomous subconscious loop (e.g., /auto on 2.0)")
    print("    /auto_gov [on|off]       : Toggle multi-observer auto-governance consensus swarm")
    ...
    print("    /download_models [Keys]  : Pre-download & cache local model presets safely without OOM")
    print("    /ensure_swarm            : Verify & ensure subconscious swarm is active before running batches")
    ...
    while True:
        try:
            if nexus.auto_subconscious_loop:
                summary, thought, uni, haptic = nexus.execute_autonomous_subconscious_cycle()
                print("\n" + "═"*76)
                print(summary)
                print("═"*76)
                time.sleep(nexus.auto_interval)
                continue
            ...
            if cmd.startswith("/"):
                ...
                if base_cmd == "/auto":
                    if arg1.lower() in ["on", "start", "enable"]:
                        nexus.auto_subconscious_loop = True
                        if arg2:
                            try: nexus.auto_interval = max(0.5, float(arg2))
                            except ValueError: pass
                        print(f" 🌀 AUTONOMOUS SUBCONSCIOUS LOOP: ENGAGED (Interval={nexus.auto_interval:.1f}s)")
                        print("    (Press Ctrl+C at any time to return to manual command mode)")
                        continue
                    elif arg1.lower() in ["off", "stop", "disable"]:
                        nexus.auto_subconscious_loop = False
                        print(" 🌀 AUTONOMOUS SUBCONSCIOUS LOOP: HALTED.")
                        continue
                    else:
                        summary, _, _, _ = nexus.execute_autonomous_subconscious_cycle(arg1)
                        print(summary)
                        continue
                elif base_cmd in ["/auto_gov", "/swarm_gov"]:
                    if arg1.lower() in ["off", "false", "disable"]:
                        nexus.auto_gov_swarm_enabled = False
                        print(" 🐝 SWARM AUTO-GOVERNANCE: DISABLED (Manual Lock Required).")
                    else:
                        nexus.auto_gov_swarm_enabled = True
                        nexus.forced_governor = None
                        print(" 🐝 SWARM AUTO-GOVERNANCE: ENGAGED (Consensus Swarm Voting Active).")
                elif base_cmd in ["/model", "/sub_model"]:
                    res = nexus.ai_interface.local_subconscious.switch_model(arg1 or "smollm")
                    print(res)
                elif base_cmd in ["/download_models", "/preload_models", "/preload"]:
                    keys = [k.strip() for k in arg1.split(",") if k.strip()] if arg1 else None
                    res_list = nexus.ai_interface.local_subconscious.preload_models(keys)
                    print("\n".join(res_list))
                elif base_cmd in ["/ensure_swarm", "/swarm_status"]:
                    res = nexus.ai_interface.local_subconscious.ensure_swarm_active()
                    print(res)
                elif base_cmd == "/add":
                    ...
                    print(res)
                elif base_cmd in ["/plugin", "/load_plugin", "/weights", "/load_weights"]:
                    res = self.load_plugin(arg1 + (" " + arg2 if arg2 else "")) if hasattr(self, 'load_plugin') else nexus.load_plugin(arg1 + (" " + arg2 if arg2 else ""))
                    print(res)
                elif base_cmd == "/vault":
                    res = nexus.rebuild_manifold(arg1 or ".")
                    print(res)
                    ...
                    summary, _, _, _ = nexus.execute_autonomous_subconscious_cycle(hint_text)
                    print(summary)
            ...
            if nexus.auto_subconscious_loop:
                nexus.auto_subconscious_loop = False
                print("\n 🌀 AUTONOMOUS SUBCONSCIOUS LOOP HALTED. (Returned to manual CLI mode)")
            else:
