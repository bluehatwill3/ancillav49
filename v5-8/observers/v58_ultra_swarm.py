...
class UltraSwarmManager:
    ...
    def route_modality(self, role):
        ...
        if not target or self.active_model == target: 
            return True
        ...
        try:
            # Check framework availability globally
            if not HF_AVAILABLE:
                print("   ❌ [SWARM] HuggingFace Transformers framework is unavailable.")
                return False

            if "moondream" in target:
                # FIX: Removed the conflicting local 'from transformers import AutoModelForCausalLM' line
